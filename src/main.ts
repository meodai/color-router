...
// Example usage / initial state setup
try {
  router.createPalette('base', {
    description: 'Core brand colors and neutral grays.',
  });
  router.set('base.primary', '#007bff'); // A vibrant blue
  router.set('base.secondary', '#6c757d'); // Muted gray
  router.set('base.accent', '#ffc107'); // Warm yellow
  router.set('base.neutral', '#f8f9fa'); // Light gray for backgrounds
  router.set('base.black', '#000000');
  router.set('base.white', '#ffffff');

  router.createPalette('theme-light', {
    extends: 'base',
    description: 'Light theme using base colors, adjusted for accessibility.',
  });
  router.set('theme-light.background', router.ref('base.neutral'));
  router.set(
    'theme-light.text',
    router.func('bestContrastWith', router.ref('theme-light.background'), router.ref('base.black')),
  );
  router.set('theme-light.link', router.ref('base.primary'));
  router.set('theme-light.button', router.ref('base.accent'));
  router.set('theme-light.buttonText', router.func('bestContrastWith', router.ref('theme-light.button')));

  router.createPalette('theme-dark', {
    extends: 'base',
    description: 'Dark theme variant.',
  });
  router.set('theme-dark.background', router.func('darken', router.ref('base.secondary'), 0.6));
  router.set(
    'theme-dark.text',
    router.func('bestContrastWith', router.ref('theme-dark.background'), router.ref('base.white')),
  );
  router.set('theme-dark.link', router.func('lighten', router.ref('base.primary'), 0.2));
  router.set(
    'theme-dark.button',
    router.func('colorMix', router.ref('base.accent'), router.ref('theme-dark.background'), 0.3),
  );
  router.set('theme-dark.buttonText', router.func('bestContrastWith', router.ref('theme-dark.button')));

  router.createPalette('semantic', {
    description: 'Semantic colors for UI states.',
  });
  router.set('semantic.success', '#28a745');
  router.set('semantic.error', '#dc3545');
  router.set('semantic.warning', '#ffc107');
  router.set('semantic.info', '#17a2b8');
  router.set('semantic.successText', router.func('bestContrastWith', router.ref('semantic.success')));
  router.set('semantic.errorText', router.func('bestContrastWith', router.ref('semantic.error')));

  router.createPalette('surface', {
    description: 'Surface colors for UI components, demonstrating relative adjustments.',
  });
  router.set('surface.background', router.ref('theme-light.background'));
  router.set('surface.onBackground', router.ref('theme-light.text'));
  router.set('surface.interaction', router.ref('theme-light.link'));
  router.set('surface.onInteraction', router.func('bestContrastWith', router.ref('surface.interaction')));
  router.set(
    'surface.line',
    router.func('relativeTo', router.ref('surface.background'), 'lch', [
      { channel: 'l', relative: -10, min: 0, max: 100 },
    ]),
  );

  // Example of a more complex function usage
  router.createPalette('complex-mix');
  router.set('complex-mix.mixedAccent', router.func('colorMix', 'base.primary', 'base.accent', 0.7, 'lch'));
  router.set(
    'complex-mix.relativeDark',
    router.func('relativeTo', 'base.secondary', 'lch', [
      { channel: 'l', relative: -20, min: 0, max: 100 },
      { channel: 'c', relative: -10, min: 0 },
    ]),
  );
  router.set(
    'complex-mix.contrasted',
    router.func('minContrastWith', 'complex-mix.relativeDark', 3, ['base.white', 'base.black']),
  );
  ...
  logEvent(`<span class="text-red-500">ERROR setting initial palettes:</span> ${(e as Error).message}`);
  console.error('Error setting initial palettes:', e);
}
